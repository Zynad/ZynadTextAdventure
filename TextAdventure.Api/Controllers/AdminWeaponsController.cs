...
public class AdminWeaponsController(IAdminWeaponService weaponService) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> GetWeapons(CancellationToken cancellationToken)
    {
        ...
        var result = await weaponService.GetAllAsync(token, cancellationToken);
    ...
    {
        var token = Request.GetAccessToken();
        var result = await weaponService.CreateAsync(token, weaponDto, cancellationToken);
        return Translate(result);
    }
    ...
    public async Task<IActionResult> UpdateWeapon(Guid id, WeaponDto weaponDto, CancellationToken cancellationToken)
    {
        ...
        var result = await weaponService.UpdateAsync(token, payload, cancellationToken);
        return Translate(result);
    }
    ...
    {
        var token = Request.GetAccessToken();
        var result = await weaponService.DeleteAsync(token, id, weaponType, cancellationToken);
        return Translate(result);
    }
