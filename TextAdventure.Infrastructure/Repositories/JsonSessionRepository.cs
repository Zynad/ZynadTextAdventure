...
{
    private readonly JsonFileStore<SessionToken> _store;

    public JsonSessionRepository(IOptions<DataStoreOptions> options, IHostEnvironment environment, ILogger<JsonSessionRepository> logger, FileConcurrencyProvider concurrencyProvider)
    {
        _store = new JsonFileStore<SessionToken>(options, environment, logger, concurrencyProvider, options.Value.SessionsFileName);
    }

    public async Task AddAsync(SessionToken sessionToken, CancellationToken cancellationToken = default)
    {
        var tokens = await _store.ReadAsync(() => new List<SessionToken>(), cancellationToken);
