...
public class JsonCharacterRepository(
    ...
    ILogger<JsonCharacterRepository> logger,
    FileConcurrencyProvider concurrencyProvider)
    : ICharacterRepository
{    
    private readonly JsonFileStore<CharacterModel> _store = new(options, environment, logger, concurrencyProvider, options.Value.CharactersFileName);

    public async Task AddAsync(Character character, CancellationToken cancellationToken = default)
    {
        var characters = await _store.ReadAsync(() => [], cancellationToken);
