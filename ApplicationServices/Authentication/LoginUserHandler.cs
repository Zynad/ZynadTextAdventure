    ...
    ILogger<LoginUserHandler> logger)
{
    public async Task<AuthResult> HandleAsync(LoginUserRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var account = await userRepository.GetByUsernameAsync(request.Identifier, cancellationToken)
                      ?? await userRepository.GetByEmailAsync(request.Identifier, cancellationToken);
        ...
        if (!authService.VerifyPassword(request.Password, account.PasswordHash, account.PasswordSalt))
        {
            return AuthResult.Unauthorized("Invalid credentials");
        }
        ...
        logger.LogInformation("Issued session for {Username}", account.Username);
