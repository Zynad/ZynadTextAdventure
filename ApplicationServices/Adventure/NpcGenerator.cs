...
public class NpcGenerator(IRandomService random)
{
    ...
    public IReadOnlyCollection<TownNpc> GenerateTownNpcs(string townName, int count, bool allowVendors = true)
    {
        ...
        {
            var given = GivenNames[random.NextInt(0, GivenNames.Length)];
            var surname = Surnames[random.NextInt(0, Surnames.Length)];
            var role = Roles[random.NextInt(0, Roles.Length)];
            var isVendor = allowVendors && random.NextDouble() < 0.2;
            ...
            generated.Add(new TownNpc
            {
                ...
                Name = $"{given} {surname}",
                Role = role,
                Personality = random.NextDouble() < 0.5 ? "Amiable" : "Reserved",
                IsVendor = isVendor
            });
        }
